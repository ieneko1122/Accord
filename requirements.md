...
> **開発方針**：各 Sprint 終了時点でアプリが「動く」状態を保つ。機能は後から追加する前提で、最小限の実装から始める。

### Sprint 1 - 認証
**ゴール**：Discord でログイン・ログアウトできる
- [ ] Discordアカウントでログイン（OAuth2）
- [ ] ログアウト（サーバーセッション破棄 + フロントPiniaクリアの両方）
- [ ] セッション管理（HttpOnly Cookie 方式、詳細は §4 参照）
- [ ] Discord snowflake ID・`@username`・`global_name`・`avatar_hash` の自動取得（ログインのたびに最新値で更新）
- [ ] プロフィール未設定の場合はプロフィール設定画面へリダイレクト

### Sprint 2 - プロフィール
**ゴール**：自分のプロフィールを登録・編集できる（ゲームタグは Sprint 5 で追加）
- [ ] プロフィール作成・編集
...
### Sprint 3 - ユーザー一覧・いいね
**ゴール**：他のユーザーを見ていいねを送れる（フィルター・ページングは Sprint 6 で追加）
...
### Sprint 4 - マッチング
**ゴール**：マッチングが成立し、相手の Discord `@username` を確認できる
- [ ] お互いにいいねしたらマッチング成立（`user1_id < user2_id` で正規化）
- [ ] マッチング一覧ページ
- [ ] マッチング成立後のみ相手の `@username` を開示（コピーボタン付き）
- [ ] 「Discordで繋がった！」ボタンで **自分側のみ** 一覧から削除（相手側の表示は維持）

### Sprint 5 - ゲームタグ（IGDB連携）
**ゴール**：好きなゲームをプロフィールに登録し、ゲームで絞り込める（アプリの目玉機能）
- [ ] IGDB APIラッパー＋Caffeineキャッシュ（`games` テーブルを経由・4 req/sec 対策）
- [ ] プロフィールにゲームタグ追加・削除（IGDBサジェスト付き、複数選択）
- [ ] ユーザー一覧でゲームタイトル絞り込み（複数タグ AND）

### Sprint 6 - UX強化
**ゴール**：大量データでも使いやすくなる
- [ ] プレイ時間帯・プレイスタイルで絞り込み
- [ ] カーソルページング（ユーザー一覧・受信いいね・マッチング一覧）
...
- [ ] テストカバレッジ整備（Service 層 TDD 漏れの補完、Controller スモークテスト）
- [ ] バグ修正・UI調整
...
| ページング | Sprint 6 で導入。一覧系 API（ユーザー一覧／受信いいね／マッチング一覧）に `?limit=20&cursor=<id>` を追加。`limit` デフォルト 20・最大 50 |
...
**開発スタイル：TDD（テストファースト）**
Service 層はテストを先に書いてから実装する。Repository 層は実装後に後追いテストでも可。

| 優先度 | 対象 | TDD |
|--------|------|-----|
| 高 | Service層（ビジネスロジック） | テストファースト |
| 中 | Repository層（DB操作） | 後追いテストでも可 |
| 低 | Controller層（基本は後回しでOK） | 後追いテストでも可。ただし OAuth2 コールバック `/login/oauth2/code/discord` と `SecurityFilterChain` 設定は例外 ― 認証バグは最重要障害につながるため WebMvcTest で最低限のスモークテストを書く |
...
- **アジャイル開発**：スプリント単位で動くものを作り続ける。「練習版を作って後で本番を作り直す」方式は採用しない
- **TDD**：Service 層はテストファーストで実装する（§6 参照）
- **カンバン方式**（GitHub Projects）：`TODO → IN PROGRESS → DONE` でタスク管理
- **ブランチ戦略**：feature ブランチで開発 → PR → main にマージ
...
| マッチング解除 | v1.0 スコープ外。マッチ済の相手を「無かったことにする」手段は提供しない（v2.0 バックログ）。ユーザーには「マッチング後の解除はできません」と UI に明記 |
...
| 一言コメント | 0〜100文字（v2.0 バックログで長い自己紹介欄に拡張予定） | 任意 |
...
| マッチング解除 | v1.0 スコープ外（UI に明記）。v2.0 バックログ |
...
> v1.0 リリース後に着手する機能・改善。Sprint 5 完了時点で優先順位を見直す。
